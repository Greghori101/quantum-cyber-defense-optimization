...
\begin{abstract}
This paper presents a hybrid quantum classical framework for cyber-defense optimization. Cyber defense is formulated as a finite-horizon state-space control problem, where the cyber state includes compromise level, attacker knowledge, vulnerability exposure, and asset criticality. An exact Linear Quadratic Regulator (LQR)-to-Quadratic Unconstrained Binary Optimization (QUBO) transformation converts the problem into a binary optimization model solved using the Quantum Approximate Optimization Algorithm (QAOA) on real IBM Noisy Intermediate-Scale Quantum (NISQ) hardware. The evaluation includes two panels. On a 16-qubit validation instance, Local QAOA and IBM QAOA achieve gaps of $0.952\%$ and $0.930\%$ relative to Constraint Programming SAT (CP-SAT), while Greedy is $210.676\%$ away. On 64, 128, and 152-qubit instances, IBM QAOA remains within $2.460\%$, $6.269\%$, and $6.623\%$ of CP-SAT. The results show that LQR-derived cyber-defense QUBOs can run on current IBM hardware with solution quality close to a strong classical reference.
...
Linear Quadratic Regulator (LQR) provides a principled framework for balancing cyber risk against defense effort. In this work, the system state includes compromise level, attacker knowledge, vulnerability exposure, and asset criticality, with vulnerability assessment motivated by the Common Vulnerability Scoring System (CVSS)~\cite{nistcvss,firstcvss}. Since defense actions are binary, the finite-horizon control problem can be formulated as a Quadratic Unconstrained Binary Optimization (QUBO) problem and solved using the Quantum Approximate Optimization Algorithm (QAOA) on Noisy Intermediate-Scale Quantum (NISQ) hardware~\cite{farhi2014qaoa,cerezo2021variational}.

To provide a meaningful evaluation, CP-SAT (Constraint Programming SAT) is adopted as the primary classical reference rather than comparing only against heuristic methods~\cite{ortoolsdoc,perron2023cpsat,lubinski2024optimization}. The experiments are divided into a validation panel comparing Greedy, CP-SAT, Local QAOA, and IBM QAOA on a common 16-qubit QUBO, and a scalability panel comparing CP-SAT and IBM QAOA on 64-, 128-, and 152-qubit instances.
...
Here $k$ denotes the discrete time step (control index) for the finite-horizon model.
...
In Eq.~\eqref{eq:state}, the matrices $A$, $B$, and $F$ have a block structure consistent with the four sub-states per node: $A$ is the state-transition matrix assembled from the $(s,\theta,v,c)$ sub-block dynamics; $B$ maps binary defenses into the attacker-knowledge and vulnerability components (affecting $\theta$ and $v$); and $F$ injects disturbances into the attacker-knowledge and vulnerability channels. The scalar parameters appearing below are interpretable: $\delta_s$ is the natural decay rate of compromise, $\beta_{\mathrm{self}}$ and $\beta_{\mathrm{net}}$ are self and network coupling strengths from attacker knowledge to compromise, $\rho_v$ couples vulnerability into compromise evolution, $\gamma_\theta$ is attacker-knowledge decay, $\theta_{\mathrm{spread}}$ controls knowledge spread over the network, $\eta_\theta$ and $\eta_v$ are control-effectiveness coefficients on knowledge and vulnerability, $\lambda_v$ is vulnerability decay, and $\alpha$ scales the disturbance into attacker knowledge. These parameters are representative and tunable to domain settings.
...
The control input reduces attacker knowledge and vulnerability exposure and the disturbance injects attacker pressure into knowledge and vulnerability. Asset criticality is modeled as slowly varying over the control horizon, so it remains constant during one optimization instance. The numeric coefficient 0.20 in the vulnerability update is a normalized disturbance-strength parameter chosen to introduce moderate attacker pressure in our experiments; it is not a physical constant and can be tuned to reflect domain-specific attacker intensity. The cost weights used in the experiments are
\begin{equation}
...
q=nH.
\end{equation}
Note that $x_k$ (Eq.~\ref{eq:state}) has dimension $4n$ (four state variables per node at a single time step), whereas the decision vector $U$ stacks the $n$ binary actions across $H$ time steps and therefore has dimension $nH$.
...
\end{equation}
The variational angles $(\gamma,\beta)$ are optimized locally using COBYLA (Constrained Optimization BY Linear Approximations)~\cite{powell1994direct}. In the validation panel, Local QAOA is run on the full QUBO. The optimized validation angles are then reused for IBM hardware execution. This design follows the near-term VQA paradigm in which a classical optimizer trains a parameterized quantum circuit~\cite{cerezo2021variational}.
