...
Cyber defense is not a one-shot classification task. A network evolves continuously as adversaries gather information, exploit vulnerable assets, and move across connected systems, so a defender must decide not only which nodes are currently compromised but also which nodes should receive limited defensive resources over a future horizon. This matches a control-theoretic view of security, in which defense actions respond to evolving information and uncertain attacker progression~\cite{miehling2019control,rasouli2014supervisory}, as also studied in moving target defense, where the defender dynamically reshapes the attack surface~\cite{zhu2013game,zhang2025mtd}.
